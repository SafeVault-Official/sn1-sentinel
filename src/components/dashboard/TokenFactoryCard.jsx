...
const emptyForm = { tokenName: '', symbol: '', supply: '' };
...
const TokenFactoryCard = ({ walletAddress, isConnected, onCreated }) => {
  ...
  const handleCreate = async (event) => {
    ...
    setStatus('Simulating token deployment...');

    try {
      const created = await tokenFactoryService.createToken({ ...form, walletAddress });
      setStatus(`Success: ${created.tokenName} (${created.symbol}) created.`);
      setForm(emptyForm);
      onCreated?.();
    } catch (error) {
  ...
  return (
    <div className="rounded-2xl border border-slate-700 bg-slate-900/80 p-4">
      <p className="text-xs uppercase tracking-wide text-slate-400">Token Factory (Mock)</p>
      <form className="mt-3 grid gap-3" onSubmit={handleCreate}>
        ...
        <input required placeholder="Symbol" value={form.symbol} onChange={(e) => updateField('symbol', e.target.value)} className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
